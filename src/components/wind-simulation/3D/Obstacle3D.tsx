...
export const Obstacle3D: React.FC<Obstacle3DProps> = ({ 
  ...
  const renderShape = () => {
    ...
    switch (obstacle.type) {
      ...
      case 'house': {
        ...
        return (
          <group position={[cx, 0, cz]}>
            ...
            {/* Pitched roof */}
            <mesh position={[0, wallH + roofH / 2, 0]} rotation={[0, 0, 0]}>
          ...
          <group position={[cx, 0, cz]}>
            ...
            {Array.from({ length: floors }).map((_, fi) => (
              [-1, 1].map(side => (
                <Box
                  key={`${fi}-${side}`}
                  args={[obstacle.width * 0.18, 1.2, 0.15]}
      ...
      // wind_generator is handled separately by WindGenerator3D
      default:
        return (
          <Box
            args={[obstacle.width, obstacle.height, obstacle.depth]}
            position={[cx, obstacle.height / 2, cz]}
            material={material}
          />
        );
    }
  ...
  // Skip rendering for wind_generator type (handled by WindGenerator3D)
